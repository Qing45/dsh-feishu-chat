...
- **设置页（设置 → 飞书）** — 在 Web 界面配置机器人 **App ID / App Secret**（持久化到 `$DSH_HOME/feishu-bot/config.json`，不会外发），并可点击切换入站消息路由到哪个工作区。
...
1. 填写机器人 **App ID** 与 **App Secret**（飞书开放平台 → 你的应用 → 凭证与基础信息），点**保存并重新连接**。
2. 选择飞书消息应路由到的工作区。
